...
## 0. Сессии 5–9 — что сделано и что осталось

### 9-я сессия (в работе) — баг-репорты с боевого роутера
Юзер накатил beta.8 на роутер. Найдено/сделано (в main):
- **`fix(engine)` — петля маршрутизации (главный баг).** `ProxyN` помечался
  `ip global` («использовать для выхода в интернет») → становился дефолтным
  подключением. У SOCKS-интерфейса (в отличие от WG) нет пиннинга endpoint'а,
  поэтому собственный трафик роутера (UDP DNS + исходящее Xray к vless-серверу)
  заворачивался `ProxyN → SOCKS → Xray → обратно в ProxyN` = петля; TCP-only
  SOCKS ещё и роняет UDP DNS. Это и есть жалоба «штормит / отваливается / ни
  один сайт не грузится / без политики заворачивает весь трафик в себя».
  Фикс: `ProxyN` теперь ТОЛЬКО цель для маршрутов по доменам (dns-proxy route,
  как AWG), global больше не ставим → WAN остаётся дефолтом, петли нет. У AWG
  global не тронут (полный WG-туннель как дефолт корректен, петли нет).
- **`feat(routes)` — редактирование маршрута.** `GET /api/routes/{id}`
  (полный список доменов/подсетей) + `PUT /api/routes/{id}`
  (`Engine.UpdateRoute`): правка имени/доменов/подсетей/цели с корректным
  тир-дауном старой формы. Юнит-тест. Веб-редактор — СЛЕДУЮЩИЙ шаг (нужен бандл).

Осталось в 9-й сессии (нужны данные/ввод юзера):
1. **Веб-бандл заблокирован npm-реестром** — до `npm ci`/`npm run build` нужен
   `RequestNetworkAccess({domain:"registry.npmjs.org"})` (запрошен, ждём). Тогда
   приземлить: (a) тумблер «Интеграция Xray» из `wip/web-xray-toggle` (только 4
   веб-файла — НЕ backend/docs версии ветки, они устарели/конфликтуют с фиксом),
   (b) веб-редактор маршрутов (кнопка «изменить» → откр. со всеми доменами).
2. **Флаг «использовать для выхода в интернет» + точный RCI-шейп** — нужен
   read-back с устройства (см. §3 плана; НЕ угадывать). Открытый вопрос: работает
   ли `dns-proxy route → ProxyN` без флага; если нет — ставить флаг на САМЫЙ
   НИЗКИЙ приоритет (WAN дефолт) + пиннинг host-route на IP сервера через WAN.
3. **nfqws «как прокси» (выбор доменов)** — нужен точный дефолтный список доменов
   от юзера (какие листы / какие домены) и уточнение «интерфейс» = UI-раздел
   выбора доменов (nfqws работает на прямом пути через NFQUEUE, туннеля не даёт).
...
3. **Веб-тумблер «Интеграция Xray»** (auto/proxy/tproxy) — код НАПИСАН и
   сохранён в ветке **`wip/web-xray-toggle`** (на origin): 4 файла —
   `web/src/pages/SettingsPage.tsx`, `web/src/lib/types.ts`,
   `web/src/i18n/pages/settings.ts`, `web/src/lib/mocks.ts`. **Бандл там НЕ
   пересобран** (в песочнице был закрыт npm-реестр `registry.npmjs.org` = 403),
   поэтому ветку НЕЛЬЗЯ мержить как есть — CI-гард «Verify committed bundle»
   упадёт. Как приземлить на main: `RequestNetworkAccess({domain:"registry.npmjs.org"})`
   → в `web/`: `npm ci` (lock НЕ менять) → `npm run build` → проверить, что
   `git diff --exit-code internal/webui/dist` пусто (кроме этой правки) →
   закоммитить `web/src` + `internal/webui/dist` ОДНИМ коммитом `feat(web)` на
   main → удалить ветку `wip/web-xray-toggle`. Бэкенд работает и без тумблера
   (auto-детект + `PUT /api/settings {xray_integration}`).
