...
Обновлено: 2026-07-09 (третья сессия). Репозиторий: `github.com/miroslavrov/keen-manager`,
...
**Эта (3-я) сессия — 4 слайса P1, все в main, сборка зелёная, бандл пересобран:**

- **`feat(lists)` авто-сплит импортируемых списков** на `user.list`/`user2.list`/…
  по ≤300 доменов (`nfqws.DefaultListSplit`, совпадает с
  `keenetic.MaxDomainsPerGroup`). `POST /api/nfqws/lists/import`
  (resolve→split→write→reload); `WriteSplit` пруннит устаревшие сиблинги, append
  мержит всю семью (`ReadListFamily`). Чистые `SplitListName/SplitDomains` +
  тесты. Диалог импорта в Bypass переведён на этот эндпоинт.
- **`feat(nfqws)` kernel-module readiness**: `nfnetlink_queue`/`xt_NFQUEUE`
  (loaded в /proc/modules или на диске под `platform.KernelModuleDirs()`).
  `NfqwsStatusView` получил `kernel_ready/missing_modules/healthy`
  (healthy = installed && running && kernel-ready). Бейдж-предупреждение в морде.
- **`feat(failover)` nfqws-guard**: `failoverTick` первым делом гоняет
  `nfqwsGuardTick`; на прямом пути (нет активного туннеля) мёртвый обход (демон
  down / нет NFQUEUE-модулей / все should-bypass домены недоступны напрямую) →
  фейловер на туннель. Поля `Failover.NfqwsGuard/NfqwsFallbackTo/
  NfqwsProbeDomains` идут через тот же `PUT /api/failover`; карточка на Failover.
- **`feat(web)`** фронт под всё выше + пересобранный `internal/webui/dist`
  (CI-гард зелёный, ребилд детерминирован — `git diff dist` пустой). tsc чистый,
  17 smoke-тестов проходят.

**ВАЖНО (поправка юзера в этой сессии):** сплит доменов = **300, НЕ 100**.
`keenetic.MaxDomainsPerGroup` остаётся 300 (прошивка принимает 300-элементные
группы). Старое «лимит ~100 строк на список» в хендоффе/роадмапе — неверно,
не снижать.
...
### Прошлая (2-я) сессия — web P1 + баги (для истории)
...
- Реальное применение маршрута: `object-group fqdn` + `dns-proxy route` на живой
  5.1.0. **Чанки остаются по 300** (`keenetic.MaxDomainsPerGroup`) — прошивка
  принимает 300-элементные группы (поправка юзера: НЕ снижать до 100). Проверить,
  что маршрут реально применяется.
- Нативный AWG2 apply: убедиться, что `WireguardN` появляется в морде и роутит.
- Проверить на железе фичи этой сессии: подхватывает ли демон `user2.list`;
  срабатывает ли nfqws-guard (переключение на туннель); корректно ли детектятся
  kernel-модули на реальной 5.1.0.

### P1 — код: сделано в этой сессии (осталась только on-device валидация)
- ✅ Авто-сплит импортируемых списков (`POST /api/nfqws/lists/import`, ≤300/файл).
- ✅ nfqws health → failover signal (`nfqwsGuardTick`, `Failover.NfqwsGuard/…`).
- ✅ Kernel-module readiness (`nfqws.KernelModulesStatus`, `…healthy`).

### P2/P3 (не сделано)
- **Дефолт-пресеты для 6 листов nfqws** (редактор есть, дефолтных доменов нет).
  Не трогал: нужен от юзера точный список «6 листов» и их дефолтные домены, чтобы
  не выдумывать контент. Кандидат на следующую сессию.
...
- Списки (resolve): `POST /api/lists/resolve {url, attr?}` (read-only fetch, safe в dry-run).
- Списки (импорт+сплит): `POST /api/nfqws/lists/import {base, url, attr?, mode?}`
  → `NfqwsImportView {base,mode,files[{name,count}],total,per_file,truncated,skipped_n,sources}`.
  Резолвит + пишет + сплитит по ≤300 (`nfqws.DefaultListSplit`), reload nfqws.
- nfqws статус: `GET /api/nfqws` и `nfqws` в `/api/state` теперь несут
  `kernel_ready`, `missing_modules[]`, `healthy` (installed&&running&&kernel).
- Failover nfqws-guard: `PUT /api/failover` (весь `model.Failover`) принимает
  `nfqws_guard`, `nfqws_fallback_to` (conn id / "direct"), `nfqws_probe_domains[]`.
