...
Обновлено: 2026-07-09 (**9-я сессия**). Репозиторий: `github.com/miroslavrov/keen-manager`,
...
### 🔴 ПРИОРИТЕТЫ СЛЕДУЮЩЕЙ СЕССИИ (кратко — детали в блоке 9-й сессии выше)
Веб-тумблер и веб-редактор маршрутов ПРИЗЕМЛЕНЫ (beta.9); npm-реестр уже одобрен
в песочнице; ветка `wip/web-xray-toggle` удалена. Осталось:
1. **P0 — снять read-back с устройства** (команды в блоке 9-й сессии). По нему:
   привести `internal/keenetic/proxyiface.go::proxyInterfaceBody` (+тест) к
   реальному шейпу (сейчас это ДОГАДКА из плана §3, изолирована в одной функции;
   отказ RCI латчит `proxyClientDown` → фолбэк в TPROXY, роутер не ломается) и
   правильно завести флаг «использовать для выхода в интернет» (низкий приоритет).
2. **P0 — on-device чек-лист** (XRAY-PROXY-PLAN §5): активация Xray → один `ProxyN`
   в «Прочие подключения → Прокси» (socks5, `127.0.0.1:10808`, connected); смена
   сервера → тот же `ProxyN`, меняется только внешний IP; Route на Xray → dns-proxy
   route → `ProxyN`, туннелит только выбранное; удаление последнего Xray → `ProxyN`
   снят. Ключевой тест «туннель несёт трафик» — `curl -x socks5h://127.0.0.1:10808`.
3. **P1 — nfqws как routable-интерфейс через `tpws` + выбор доменов** (спец в блоке
   9-й сессии: зеркалить Xray-Proxy-плумбинг; общий источник доменов с Routes;
   дефолт-сид Discord+YouTube из `internal/presets`).
